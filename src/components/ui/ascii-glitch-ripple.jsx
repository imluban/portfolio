...
export function AsciiGlitchRipple({
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      const animate = () => {
        ...
        stateRef.current.waves = stateRef.current.waves.filter((w) => t - w.startTime < stateRef.current.dur);
  ...
  return (
    <Component
      ref={elRef}
      className={cn(
  "select-none relative inline-block transition-colors duration-300",
  className
)}
      {...props} />
  );
